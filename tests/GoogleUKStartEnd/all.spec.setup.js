...
// -------------------------------------------------------------------------
// -------------------------------------------------------------------------
describe('Pre-test setup for GoogleUKStartEnd', function () {
    it("Reset PLACEHOLDERs for GoogleUKStartEnd test script", function () {
        var sKey = "";
        var sReplacement = "";

        try {
            var TestData = require("../../tests/TestData/" + browser.params.testEnv + ".GoogleUKStartEnd.data.js");
            ...
            switch(dicKey.substr(1, 1).toUpperCase()){
                case "{":
                    ...
                    replace({
                        ...
                        paths: ["./tests/TestData/" + browser.params.testEnv + ".GoogleUKStartEnd.data.js"],
                        recursive: false,
                        silent: true
                    });
    ...
    it("Set PLACEHOLDERs for GoogleUKStartEnd test script", function () {
        var sReplacement = "";
        ...
        try {
            delete require.cache[require.resolve("../../tests/TestData/" + browser.params.testEnv + ".GoogleUKStartEnd.data.js")];
            var TestData = require("../../tests/TestData/" + browser.params.testEnv + ".GoogleUKStartEnd.data.js");
            var testData = new TestData();
            testData.load(dictData);
        } catch(err) {
